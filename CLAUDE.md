...
- Code pushed to local git repo (`main` branch)
- Core design system established (dark theme, yellow accent, Bebas Neue / IBM Plex fonts)
...
- [ ] Bundle ID needs updating from `com.drafthouse.coat` → `com.drafthouse.buildout`
- [ ] Rebrand: rename app from COAT → Buildout, Paint Calculator becomes "Paint" tab

### Next Immediate Tasks

1. Update bundle ID in `app.json` to `com.drafthouse.buildout`
1. Add `@react-navigation/bottom-tabs` and `@react-navigation/material-top-tabs` — two-section nav required
1. Restructure `App.js` into Calculate / Quote top-level sections
1. Build Tile Calculator (second tool, first new screen)
1. Begin Quoting module (QuoteScreen + QuoteHistoryScreen)
...
|2|Tile Calculator     |🔲 Not started|Tile     |
|3|Grout Calculator    |🔲 Not started|Grout    |
|4|LVP Calculator      |🔲 Not started|LVP      |
|5|Carpet Calculator   |🔲 Not started|Carpet   |
|6|Staircase Calculator|🔲 Not started|Stairs   |
|7|Drywall Calculator  |🔲 Not started|Drywall  |
...
### Future (V2 + separate apps)

- Deep link from calculator results → Quote builder (e.g. "Add to Quote" button on results card pre-fills a line item)
...
```
Joint volume    = tile perimeter × joint depth × joint width (per tile)
Tile perimeter  = 2 × (tile length + tile width)  in inches
Total volume    = joint volume × number of tiles
Bags needed     = ceil(total volume / coverage per bag)
Coverage varies by grout type — sanded: ~25 sq ft/bag, unsanded: ~50 sq ft/bag
Use sanded grout for joints > 1/8 inch, unsanded for joints ≤ 1/8 inch
With waste      = bags needed × 1.10
```

### Drywall Calculator

```
Wall area       = sum of (width × height) per wall
Ceiling area    = length × width (if drywalling ceiling)
Total area      = wall area + ceiling area
Door deduction  = 20 sq ft per door
Window deduction= 15 sq ft per window
Sheets needed   = ceil(net area / sheet area)  — standard sheet: 4×8 = 32 sq ft, 4×12 = 48 sq ft
With waste      = ceil(sheets needed × 1.10)
Screws (approx) = sheets needed × 28  (28 screws per 4×8 sheet rule of thumb)
Joint compound  = ceil(total area / 100)  — approx 1 bucket per 100 sq ft
```

### Staircase Calculator

```
Tread area     = tread depth × stair width × number of stairs
Riser area     = riser height × stair width × number of stairs  (if carpeting risers)
Total area     = tread area + riser area (if applicable)
With waste     = total area × 1.15  — stairs have more cuts, use 15%
Boxes/rolls    = ceil(total area with waste / coverage per unit)
```
...
|Keywords |home renovation calculator, paint, tile, flooring, contractor, job quote, estimate, LVP, carpet, drywall|
...
- [ ] All 7 calculator tools built and tested
...
1. Upload metadata, screenshots (min 2, max 5 — show at least 3 different tools)
